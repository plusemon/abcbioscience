                    ...
                    <table id="laravel_datatable" class="table table-striped table-bordered table-td-valign-middle"
                        ...
                        <tbody>
                            @foreach ($collections as $collection)
                                <tr>
                                    <td>
                                        <input type="checkbox" name="fee_id[]" value="{{ $collection->id }}"
                                               class="check_single_class" id="{{ $collection->id }}" />
                                    </td>
                                    <td>
                                        {{ $loop->iteration }}
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        <a href="{{ route('admin.feeCollectionDestory', $collection->id) }}" id="delete"
                                        ...
                                        @endif
                                        {{-- <a href="#" class="btn btn-success" data-toggle="modal"
                                           data-target="#exampleModal_{{ $collection->id }}"> <i class="fa fa-money-bill"></i> Make
                                            Payment</a> --}}


                                        <!-- Modal -->
                                        {{-- <div class="modal fade" id="exampleModal_{{ $collection->id }}" tabindex="-1"
                                             aria-labelledby="exampleModalLabel" aria-hidden="true">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <form action="{{ route('admin.student.due.invoice.store', $collection->id) }}"
                                                          method="post">
                                                        @csrf
                                                        ...
                                                        <div class="modal-body">
                                                            ...
                                                            <input type="text" class="form-control" name="transaction_id" value=""
                                                                   id="transaction_id"
                                                                   placeholder="Enter Transaction ID/Mobile Number" required>
                                                            <div class="text-danger"> {{ $errors->first('transaction_id') }}
                                                            </div>

                                                            <br>
                                                            <label>Payment Date</label>
                                                            <input type="date" class="form-control" name="created_at" value=""
                                                                   id="created_at" placeholder="Date" required>
