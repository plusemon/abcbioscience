    ...
    <div id="content" class="content">
        <div class="panel panel-inverse">
            ...
            <div class="panel-body">
                ...
                <form action="{{ route('admin.mcq-setting.update', $examSet->id) }}" method="post">
                    @csrf
                    @method('put')
                    <div class="row" >
                            ...
                            <div class="form-group">
                                ...
                                <select name="batch_setting_id" id="" class="batch_setting_id form-control" required>
                                    ...
                                    @foreach($batches as $batch)
                                        <option {{ $examSet->batch_setting_id == $batch->id ? 'selected' :'' }} value="{{ $batch->id }}"> {{ $batch->batch_name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-xs-12 col-sm-6 col-md-3">
                            <div class="form-group">
                                ...
                                <select name="batch_type_id" id="batch_type_id" class="batch_type_id form-control" required>
                                    ...
                                    @foreach($batchTypies as $batchType)
                                        <option {{ $examSet->batch_type_id == $batchType->id ? 'selected' :'' }} value="{{ $batchType->id }}"> {{ $batchType->name }}</option>
                                    @endforeach
                                </select>
                          ...
                          <div class="col-xs-12 col-sm-6 col-md-4">
                            <div class="form-group">
                                <label for="Session">Exam Start Date & Time :</label>
                                <input type="datetime-local"  value="{{ $examSet->exam_start_date_time->format('Y-m-d\TH:i:s') }}" name="exam_start_date_time" class="form-control" required/>
                                <div class="text-danger">{{ $errors->first('exam_start_date_time') }}</div>
                            </div>
                        </div>
                        <div class="col-xs-12 col-sm-6 col-md-4">
                            <div class="form-group">
                                <label for="Session">Exam End Date & Time :</label>
                                <input type="datetime-local" value="{{ $examSet->exam_end_date_time->format('Y-m-d\TH:i:s') }}" name="exam_end_date_time" class="form-control" required/>
                                <div class="text-danger">{{ $errors->first('exam_end_date_time') }}</div>
                            </div>
                        </div>
                        <div class="col-xs-12 col-sm-6 col-md-4">
                            <div class="form-group">
                                <label for="Session"> Duration : ( Minutes )</label>
                                <input type="text" value="{{ $examSet->duration }}" name="duration" class="form-control" required/>
                        ...
                        <div class="col-xs-12 col-sm-6 col-md-3">
                            <div class="form-group">
                                <label for="Batch Setting">Result view :</label>
                                <select name="result_view" class="form-control">
                                    <option value="0">Exam time finished</option>
                                    <option value="1" {{ $examSet->result_view ? 'selected':'' }}>Instant</option>
                                </select>
                                <div class="text-danger">{{ $errors->first('result_view') }}</div>
                            </div>
                        </div>
                        ...
                        <div class="col-xs-12 col-sm-6 col-md-3">
                            <div class="form-group">
                                ...
                                <select name="fee_cat_id" id="pay_time_id" class="form-control" disabled>
                                    <option value="">Fee Category</option>
                                    @foreach($fee_categories as $fee_cat)
                                    <option {{ $examSet->fee_cat_id == $fee_cat->id ? 'selected' :'' }} value="{{ $fee_cat->id }}">{{ $fee_cat->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-xs-12 col-sm-6 col-md-3">
                            <div class="form-group">
                                ...
                                <select name="pay_time_id" id="pay_time_id" class="form-control" required>
                                    ...
                                    <option {{ $paytime_id == $paytime->id ? 'selected' :'' }} value="{{ $paytime->id }}">{{ $paytime->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-xs-12 col-sm-6 col-md-3">
                            <div class="form-group">
                                ...
                                <input type="text" value="{{ $examSet->amounts->amount ?? ''  }}" name="amount" class="form-control" placeholder="Amount"/>
