...
@section('title','Diagram Questions List')
@section('content')

    <div id="content" class="content">
        <div class="panel panel-inverse">
            <div class="panel-heading">
                <h4 class="panel-title">Diagram Questions List </h4>
            ...
            <div class="panel-body">
                ...
                <table id="laravel_datatable" class="table table-striped table-bordered table-td-valign-middle">
                    ...
                    @foreach($questions as $question)
                        <tr>
                            <td>{{ $loop->index+1}}</td>
                            <td>{{ $question->diagramQuestionSubjects?$question->diagramQuestionSubjects->question_no:NULL}}</td>
                            <td>{{ $question->subjects?$question->subjects->name:NULL}}</td>
                            <td>{{ $question->diagramQuestionSubjects?$question->diagramQuestionSubjects->topic:NULL}}</td>
                            ...
                            <td class="d-flex btn-group">
                                    <a href="{{ route('admin.diagram-setting.edit', $question->id) }}?qid={{ $question->question_subject_id }}"
                                        class="btn btn-outline-success btn-sm d-flex align-items-center "> <i
                                            class="fa fa-edit"></i> Edit</a>
                                    <a href="{{ route('admin.diagram.question.student.setting.create','qtype=diagram&qid='.$question->id)}}"
                                        class="btn btn-outline-success"> Setting</a>



                                        <form action="{{ route('admin.diagram-setting.destroy',$question->id)  }}" method="post">
                                            @csrf
                                            @method('DELETE')
