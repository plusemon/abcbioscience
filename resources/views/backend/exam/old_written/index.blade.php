    ...
    <div id="content" class="content">
        <div class="panel panel-inverse">
            <div class="panel-heading">
                <h4 class="panel-title">Written Exam Results List</h4>
            ...
            <div class="panel-body">
                ...
                <form action="">
                    <div class="row">
                        ...
                        <div class="form-group col-md-4 ">
                            <label>Exam</label>
                            <select name="exam" class="form-control" id="examList"
                                onchange="getResults(this.value)"></select>
                        </div>
                ...
                @if ($exam)
                    <div class="bg-light border py-2 text-center" style="position: relative;">
                        ...
                        <button onclick="window.open('{{ request()->fullUrl() }}&print=true','blank')"
                            class="btn btn-black" style="position: absolute; right:20px;">Print</button>
                        ...
                        <h4>Exam hold on : {{ date('d-m-Y h:i A',strtotime($exam->exam_start_date_time)) }} to {{ date('d-m-Y h:i A',strtotime($exam->exam_end_date_time)) }} </h4>
                    </div>
                @endif


                <form action="{{ route('admin.written.exam.mark.entry') }}" method="post" accept-charset="utf-8">
                     @csrf
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            ...
                                            <input type="hidden" name="written_exam_result_id[]" value="{{ $student->resultId }}">
                                        @endif
                                    </td>
                                        <td><span class="text-success">Present</span></td>
                                        <td>
                                              <input type="text" name="result[]" placeholder="Enter Written Exam Mark" class="form-control">
            ...
            if (batchId) {
                $.ajax(url)
                    .then(res => {
                        ...
                        res.forEach(item => {
                            examList.append(
                                `<option value="${item.id}">${item.written_question_subjects.question_no}</option>`
                            );
                        })
                    })
                    .catch(e => console.log(e))
