...
@section('title', 'Offline MCQ Exam Results')
@section('content')

    <div id="content" class="content">
        <div class="panel panel-inverse">
            <div class="panel-heading">
                <h4 class="panel-title">Offline MCQ Exam Results List</h4>
            ...
            <div class="panel-body">
                ...
                <form action="{{ route('admin.offlinemcq.exam.mark.entry') }}" method="post" accept-charset="utf-8">
                    @csrf
                                        ...
                                        @if ($student->resultId)
                                            <td>
                                                ...
                                                @else
                                                    ...
                                                    <input type="hidden" name="offline_exam_result_id[]"
                                                           value="{{ $student->resultId }}">
                                                @endif
                                            </td>
                                            <td><span class="text-success">Present</span></td>
                                            <td>
                                                <input type="text" name="result[]" placeholder="Enter Offline MCQ Exam Mark"
                                                       class="form-control">
            ...
            if (batchId) {
                $.ajax(url)
                    .then(res => {
                        ...
                        res.forEach(item => {
                            examList.append(
                                `<option value="${item.id}">${item.offline_mcq_question_subjects.question_no}</option>`
                            );
                        })
                    })
                    .catch(e => console.log(e))
