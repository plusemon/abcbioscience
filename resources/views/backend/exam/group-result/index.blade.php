                ...
                <div class="panel-body">
                    ...
                    <form action="{{ route('admin.result.group.store') }}" method="post" accept-charset="utf-8" >
                        ...
                        <div class="form-inline">
                            ...
                            <select name="mcq_exam_type" id="mcq_exam_type" class="mcq_exam_type form-control mr-3" >
                                 <option  value="">Select Type</option>
                                 <option  value="1">Online MCQ</option>
                                 <option  value="2">Offline MCQ</option>
                            </select>
                            ...
                            <select name="offlinemcq_exam_setting_id" id="offlinemcq_exam_setting_id" class="offlinemcq_exam_setting_id form-control mr-3" >
                                 <option  value="">Select Offline MCQ Exam</option>
                            </select>
                     ...
                     <table class="table table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $group->offlinemcqexamsetting->OfflineMcqQuestionSubjects?$group->offlinemcqexamsetting->OfflineMcqQuestionSubjects->question_no:'' }}</td>
                                @endif
...
@section('customjs')
    ...
    <script>
        $(document).ready( function () {
         ...
         $('#mcq_exam_setting_id').hide();
         $('#written_exam_setting_id').hide();
         $('#offlinemcq_exam_setting_id').hide();
          
          
         $('#mcq_exam_type').on('change', function (){
             var mcq_exam_type = $('#mcq_exam_type').val();
             ...
             if(mcq_exam_type == 1)
             {
                $('#mcq_exam_setting_id').show();
                $('#written_exam_setting_id').show();
                $('#offlinemcq_exam_setting_id').hide();
             }
             else if(mcq_exam_type == 2){
                 
                 $('#mcq_exam_setting_id').hide();
                 $('#written_exam_setting_id').show();
                 $('#offlinemcq_exam_setting_id').show();
             }
        ...
        function getBatchSetting()
        {
         ...
         function geOfflineMcqExamSetting()
         {
              var class_id    = $('.class_id').val();
              var session_id  = $('.session_id').val();
              var batch_setting_id  = $('.batch_setting_id').val();
                ...
                if(class_id && session_id && batch_setting_id)
                {
                    $.ajax({
                        type: "get",
                        url: "{{ route('admin.result.get.exam.setting.offlinemcq') }}",
                        data: {class_id:class_id,session_id:session_id,batch_setting_id:batch_setting_id},
                        success: function (data) {
                            if(data.status == true)
                            {
                                $(".offlinemcq_exam_setting_id").html(data.batch_setting);
                            }
                        },
                        error: function (data) {
                            console.log('Error:', data);
