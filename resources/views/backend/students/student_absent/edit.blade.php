                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-12">
                                                ...
                                                <input type="hidden" id="student_id"  value={{$absentStudent->user_id}}"" class="student_id"/>
                                                <div class="form-group dropdown">
                                                    <label for="">Student name </label>
                                                    <input autocomplete="off" type="text" id="p_name_sku_bar_code_id" value=" {{$absentStudent->students?$absentStudent->students->user?$absentStudent->students->user->name:NULL:NULL}}" class="form-control studentName" placeholder="Student name">
                                            ...
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    <label >Batch Name ({{ $absentStudent->batchsetting?$absentStudent->batchsetting->batch_name:NULL }})</label>
                                                    <select name="batch_setting_id" id="batchSettingId" class="batch_setting_id form-control" required>
                                                        <option value="">Select Batch </option>
                                                    </select>
                                                </div>
                                                <input type="hidden" class="existingBatchSetting" value="{{$absentStudent->batch_setting_id}}" />
                                            </div>
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    <label >Batch Type ({{ $absentStudent->batchTypes?$absentStudent->batchTypes->name:NULL }})</label>
                                                     <select name="batch_type_id" id="batchTypeID" class="batch_type_id form-control" required>
                                                        <option value="">Select Batch Type </option>
                                                    </select>
                                                </div>
                                                <input type="hidden" class="existingBatchType" value="{{$absentStudent->batch_type_id}}" />
                                            </div>
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label for="" >Class ({{ $absentStudent->classes?$absentStudent->classes->name:NULL }})</label>
                                                    <input type="text" disabled class="class_id form-control" />
                                                </div>
                                            </div>
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label >Session ({{ $absentStudent->sessiones?$absentStudent->sessiones->name:NULL }})</label>
...
@section('customjs')
    ...
    /*get batch id by student id , class id and session id*/
        ...
        $(document).ready(function(){
            getDefaultBatchSetting();
            getDefaultBatchType();
            getClassSession();
        });


        /*get default batch setting id*/
        function getDefaultBatchSetting()
        {
            var exist = $('.existingBatchSetting').val();
            var user_id         = $('.student_id').val();
            if(user_id.length > 0)
            {
                var url  = "{{route('admin.getStudentBatch')}}";
                $.ajax({
                        ...
                        success: function(response)
                        {
                            ...
                            {
                                $('.batch_setting_id').html(response.data);
                                $('#batchSettingId option[value="'+exist+'"]').attr("selected", "selected");
                            }else{
                                $('.batch_setting_id').html('<option value="">Select Student First</option>');
                            }
                        },
        ...
        function getDefaultBatchType()
        {
            var existBatchTypeId        = $('.existingBatchType').val();
            var user_id                 = $('.student_id').val();
            var batch_setting_id        = $('.existingBatchSetting').val();
            ...
            if(user_id.length > 0 && batch_setting_id)
            {
                ...
                $.ajax({
                        ...
                        success: function(response)
                        {
                            ...
                            {
                                $('.batch_type_id').html(response.data);
                                $('#batchTypeID option[value="'+existBatchTypeId+'"]').attr("selected", "selected");
                            }else{
                                $('.batch_type_id').html('<option value="">Select Batch First</option>');
                            }
                        },
        ...
        {
            var user_id              = $('.student_id').val();
            var batch_type_id        = $('.existingBatchType').val();
            var batch_setting_id     = $('.existingBatchSetting').val();
