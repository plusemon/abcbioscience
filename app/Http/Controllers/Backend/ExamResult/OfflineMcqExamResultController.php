...
class OfflineMcqExamResultController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            if ($classId && $sessionId && $batchId) {
                ...
                return ExamSetting::whereStatus(1)
                    ...
                    ->whereHas('OfflineMcqQuestionSubjects')
                    ->with('OfflineMcqQuestionSubjects')
                    ->where('fee_cat_id',8)
                    ->get();
        ...
        if ($examId) {
            ...
            $data['students'] = Student::where('activate_status',1)
                ...
                ->map(function ($student) use ($examId) {
                    $result = $student->offlinemcq_result($examId);
        ...
        if ($request->has('print')) {
            return view('backend.exam.offlinemcq.print', $data);
        ...
        if ($request->has('sms')) {
            ...
            return view('backend.exam.offlinemcq.sms', $data);
        }

        $data['classes'] = Classes::whereStatus(1)->get();
        $data['sessions'] = Sessiones::whereStatus(1)->get();
        return view('backend.exam.offlinemcq.index', $data);
    }
    ...
    {
       return $data['result'] = OfflineMcqExamResult::findOrFail($id);
        return view('backend.exam.offlinemcq.show', $data);
    }

    public function edit(OfflineMcqExamResult $offlineMcqExamResult)
    {
        //
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $result = OfflineMcqExamResult::findOrFail($id);
    ...
    public function writtenmarkentry(Request $request)
    {
       ...
       foreach(array_chunk($input['student_id'],1000000) as $x){
        ...
        foreach($x as $key => $value)
        {
            ...
            $checkresult = OfflineMcqExamResult::where('exam_setting_id',$request->exam_setting_id)
            ...
            { 
                if(($input['result'][$key]) != null){
                    ...
                    $findreslt  = OfflineMcqExamResult::where('student_id',$input['student_id'][$key])->where('exam_setting_id',$exam->id)->first();
                     
                    $data = OfflineMcqExamResult::find($findreslt->id);
                ...
                if(($input['result'][$key]) != null){
                    ...
                    $result = new OfflineMcqExamResult();
    ...
    {
        WrittenExamResult::find($id)->delete();
        $notification = array(
