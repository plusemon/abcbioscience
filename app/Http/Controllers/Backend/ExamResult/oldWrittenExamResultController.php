...
class WrittenExamResultController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            if ($classId && $sessionId && $batchId) {
                ...
                return ExamSetting::whereStatus(1)
                    ...
                    ->whereHas('writtenQuestionSubjects')
                    ->with('writtenQuestionSubjects')
                    ->where('fee_cat_id',5)
                    ->get();
        ...
        if ($examId) {
            ...
            $data['students'] = Student::where('activate_status',1)
                ...
                ->map(function ($student) use ($examId) {
                    $result = $student->written_result($examId);
        ...
        if ($request->has('print')) {
            return view('backend.exam.written.print', $data);
        }
        ...
        if ($request->has('sms')) {
            ...
            return view('backend.exam.written.sms', $data);
        }

        $data['classes'] = Classes::whereStatus(1)->get();
        $data['sessions'] = Sessiones::whereStatus(1)->get();
        return view('backend.exam.written.index', $data);
    }
    ...
    {
        $data['result'] = WrittenExamResult::findOrFail($id);
        return view('backend.exam.written.show', $data);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $result = WrittenExamResult::findOrFail($id);
    ...
    public function writtenmarkentry(Request $request)
    {
       ...
       foreach(array_chunk($input['student_id'],1000000) as $x){
        ...
        foreach($x as $key => $value)
        {
            ...
            $checkresult = WrittenExamResult::where('exam_setting_id',$request->exam_setting_id)
            ...
            if($checkresult>0)
            { 
                if(!empty($input['result'][$key])){
                    
                    $findreslt  = WrittenExamResult::where('student_id',$input['student_id'][$key])->where('exam_setting_id',$exam->id)->first();
                     
                    $data = WrittenExamResult::find($findreslt->id);
