...
class Student extends Model
{
    ...
    public function checkExistingForWrittenStudent($exam_setting_id)
    {
     ...
     public function checkExistingForDiagramStudent($exam_setting_id)
    {
        return StudentQuestionSetting::where('student_id',$this->id)
        ->where('batch_setting_id',$this->batch_setting_id)
        ->where('batch_type_id',$this->batch_type_id)
		...
		public function montlyFeeAmountID($fee_cat_id,$class_id,$session_id,$batch_setting_id,$batch_type_id)
		{
			$fee_amount_setting_id =  FeeAmountSetting::whereNull('deleted_at')
				->where('class_id',$class_id)
				->where('session_id',$session_id)
				->where('batch_setting_id',$batch_setting_id)
				->where('batch_type_id',$batch_type_id)
		...
		public function offlinemcq_result($exam_setting_id)
		{
			 return OfflineMcqExamResult::where('exam_setting_id', $exam_setting_id)
			->where('student_id', $this->id)
			->first();
		}
		
		
		public function diagram_result($exam_setting_id)
		{
			 return DiagramExamResult::where('exam_setting_id', $exam_setting_id)
			->where('student_id', $this->id)
			->first();
		}
