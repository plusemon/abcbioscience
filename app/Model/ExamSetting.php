...
use App\Models\OfflineMcqQuestion;
...
class ExamSetting extends Model
{
	...
	public function offlinemcqresult()
	{
		return $this->hasOne(OfflineMcqExamResult::class, 'exam_setting_id');
	}
	
	
    public function diagramresult()
	{
		return $this->hasOne(DiagramExamResult::class, 'exam_setting_id');
	}
	...
	public function diagramQuestionSubjects()
	{
		return $this->belongsTo(DiagramQuestion::class, 'question_subject_id', 'id')->whereNull('deleted_at');
	}

    public function OfflineMcqQuestionSubjects()
	{
		return $this->belongsTo(McqOfflineQuestion::class, 'question_subject_id', 'id')->whereNull('deleted_at');
	}
	...
	public function offlinemcqexam()
	{
		return $this->belongsTo(OfflineMcqQuestion::class,'question_subject_id');
	}
	...
	public function diagramexam()
	{
		return $this->belongsTo(DiagramQuestion::class, 'subject_id');
	}
    ...
    public function is_offlinemcq_exam_completed()
	{
		$userId = Auth::id();
		return OfflineMcqExamResult::where('created_by', $userId)
			->where('exam_setting_id', $this->id)
			->whereStatus(1)
			->count();
	}
